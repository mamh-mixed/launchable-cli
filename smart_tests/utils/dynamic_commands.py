...
class DynamicCommandBuilder:
    ...
    def create_detect_flakes_commands(self, base_app: typer.Typer,
                                      base_callback_func: Callable,
                                      base_callback_options: Dict[str, Any]) -> None:

        detect_flakes_functions = self.registry.get_detect_flakes_functions()
        for test_runner_name, test_runner_func in detect_flakes_functions.items():
            combined_command = self._create_combined_detects_flakes_command(
                test_runner_name,
                test_runner_func,
                base_callback_func,
                base_callback_options
            )
            base_app.command(name=test_runner_name, help=f"Detect flaky tests using {test_runner_name}")(combined_command)
    ...
    # Note(Konboi): copy from _create_combined_subset_command
    def _create_combined_detects_flakes_command(self, test_runner_name: str, test_runner_func: Callable,
                                                base_callback_func: Callable, base_callback_options: Dict[str, Any]) -> Callable:
        """Create a combined detect flakes command for a specific test runner."""
        ...
        # Set the signature for the combined function
        setattr(combined_function, '__signature__', inspect.Signature(combined_params))
        combined_function.__name__ = f"detect_flakes_{test_runner_name.replace('-', '_')}"
